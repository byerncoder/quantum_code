...
1. 存在经典上互相排斥（互相正交）的两个态，分别编码为**$\lvert0\rangle$**（0态）、**$\lvert1\rangle$**（1态）
2. 能够制备系统处在这两个态的叠加态。即在保持态$\lvert\psi\rangle$不被破坏的情况下，没有任何物理方法可以确定或区分在这个态中系统究竟处在$\lvert0\rangle$态或$\lvert1\rangle$态。
